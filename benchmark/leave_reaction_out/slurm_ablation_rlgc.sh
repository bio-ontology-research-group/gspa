...
#SBATCH -t 24:00:00
#SBATCH -J lro_rlgc
#SBATCH -o /data/hohndor/gspa/proteomes/bench_ecoli/leave_reaction_out/logs/lro_rlgc_%A_%a.out
#SBATCH -e /data/hohndor/gspa/proteomes/bench_ecoli/leave_reaction_out/logs/lro_rlgc_%A_%a.err
...
OUT=$LRO/runs_rlgc/${MODE}/shard_${ID}
mkdir -p $OUT $LRO/logs

echo "== RLGC mode=$MODE shard=$ID start=$START end=$END =="

python3 $LRO/run_ablation_rlgc.py \
    --cases $CASES \
    --root $ROOT \
    --tag $TAG \
    --jar $JAR \
